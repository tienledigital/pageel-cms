...
const Dashboard: React.FC<DashboardProps> = ({
  ...
  const [isPickerOpen, setIsPickerOpen] = useState<"posts" | "images" | null>(
    null,
  );
    ...
    try {
      ...
      try {
        if (effectiveImagesPath) {
          const imageContents =
            await gitService.getRepoContents(effectiveImagesPath);
          imageCount = imageContents.filter(
            (item) => item.type === "file",
          ).length;
        }
      } catch {
    ...
    reader.onload = async (e) => {
      try {
        ...
        Object.entries(importedConfig).forEach(([key, value]) => {
          ...
          const storageKey =
            key === "pageel-cms-lang" ? key : `${key}_${repo.full_name}`;
        ...
        try {
          const configObject = {
            ...
            settings: {
              postFileTypes:
                importedConfig.postFileTypes || settings.postFileTypes,
              imageFileTypes:
                importedConfig.imageFileTypes || settings.imageFileTypes,
              publishDateSource:
                importedConfig.publishDateSource || settings.publishDateSource,
              imageCompressionEnabled:
                importedConfig.imageCompressionEnabled ??
                settings.imageCompressionEnabled,
              maxImageSize:
                importedConfig.maxImageSize || settings.maxImageSize,
              imageResizeMaxWidth:
                importedConfig.imageResizeMaxWidth ||
                settings.imageResizeMaxWidth,
            },
            commits: {
              newPost: importedConfig.newPostCommit || settings.newPostCommit,
              updatePost:
                importedConfig.updatePostCommit || settings.updatePostCommit,
              newImage:
                importedConfig.newImageCommit || settings.newImageCommit,
              updateImage:
                importedConfig.updateImageCommit || settings.updateImageCommit,
            },
          };
        ...
        let message = t(
          "dashboard.settings.importExport.importError.validation",
        );
  ...
  const handleDeleteConfig = async () => {
    ...
    try {
      ...
      const repoSpecificKeys = [
        ...
        `postTableColumns_${prefix}`,
        `postTableColumnWidths_${prefix}`,
        `postFileTypes_${prefix}`,
        `imageFileTypes_${prefix}`,
        `publishDateSource_${prefix}`,
        `imageCompressionEnabled_${prefix}`,
        `maxImageSize_${prefix}`,
        `imageResizeMaxWidth_${prefix}`,
        `newPostCommit_${prefix}`,
        `updatePostCommit_${prefix}`,
        `newImageCommit_${prefix}`,
        `updateImageCommit_${prefix}`,
      ];
  ...
  const getPageIcon = () => {
    switch (activeView) {
      case "dashboard":
        return <DatabaseIcon className="w-8 h-8 text-notion-text mr-3" />;
      case "images":
        return <ImageIcon className="w-8 h-8 text-notion-text mr-3" />;
      case "template":
        return <TemplateIcon className="w-8 h-8 text-notion-text mr-3" />;
      case "workflows":
        return <BoardIcon className="w-8 h-8 text-notion-text mr-3" />;
      case "backup":
        return <DownloadIcon className="w-8 h-8 text-notion-text mr-3" />;
      case "settings":
        return <SettingsIcon className="w-8 h-8 text-notion-text mr-3" />;
      default:
        return <DocumentIcon className="w-8 h-8 text-notion-text mr-3" />;
    }
  };
  ...
  const getPageTitle = () => {
    switch (activeView) {
      ...
      case "images":
        return t("dashboard.nav.manageImages");
      case "template":
        return t("dashboard.nav.template");
      case "workflows":
        return t("dashboard.nav.workflows");
      case "backup":
        return t("dashboard.nav.backup");
      case "settings":
    ...
    <div className="flex h-screen bg-white font-sans overflow-hidden text-notion-text">
          ...
          <div className="bg-white border-b border-notion-border px-4 py-2 flex items-center justify-center sticky top-0 z-40 shadow-sm animate-fade-in text-xs">
            <div className="flex items-center text-yellow-600">
              ...
              <span className="font-medium">
                {t("dashboard.syncWarning.description")}
              </span>
            </div>
          </div>
        )}
        ...
        {isSyncing && (
          <div className="bg-blue-50 border-b border-blue-200 px-4 py-2 flex items-center justify-center sticky top-0 z-40 shadow-sm animate-fade-in text-xs">
            <div className="flex items-center text-blue-700">
              ...
              <span className="font-medium">
                {useAppStore.getState().syncMessage || "Syncing..."}
              </span>
            </div>
          </div>
        )}
        ...
        <div className="lg:hidden flex items-center justify-between bg-white border-b border-notion-border p-4">
          <div className="flex items-center overflow-hidden">
            <button
              ...
              className="p-1 -ml-1 rounded-sm text-gray-500 hover:bg-gray-100 focus:outline-none flex-shrink-0 mr-2"
            >
        ...
        <main className="flex-1 overflow-y-auto bg-white">
          <div
            className={`w-full mx-auto py-8 ${
              ...
              activeView === "images" ||
              activeView === "template"
                ? "px-4 sm:px-6 lg:px-6" /* Data views: tighter padding for full-width tables */
                : "px-6 sm:px-8 lg:px-12" /* Form views: wider padding, centered content */
            }`}
          >
      ...
      {isPickerOpen && (
        <DirectoryPicker
          ...
          initialPath={
            isPickerOpen === "posts" ? settings.postsPath : settings.imagesPath
          }
        />
      )}
